...
namespace LoL_URF_HybridStats_vs12.Helper
{
        ...
        INotifyPropertyChanged
    {
        ...
        {
            _siInfo = info;
         /*   _keyedEntryCollection = (KeyedDictionaryEntryCollection<TKey>)info.GetValue("_keyedEntryCollection", typeof(KeyedDictionaryEntryCollection<TKey>));
            _countCache = (int)info.GetValue("_countCache", typeof(int));
            _dictionaryCache = (Dictionary<TKey, TValue>)info.GetValue("_dictionaryCache", typeof(Dictionary<TKey, TValue>));
            _dictionaryCacheVersion = (int)info.GetValue("_dictionaryCacheVersion", typeof(int));
            _version = (int)info.GetValue("_version", typeof(int));*/
        }

      /*  public void GetObjectData(SerializationInfo info, StreamingContext ctxt)
        {
            info.AddValue("_keyedEntryCollection", this._keyedEntryCollection);
            info.AddValue("_countCache", this._countCache);
            info.AddValue("_dictionaryCache", this._dictionaryCache);
            info.AddValue("_dictionaryCacheVersion", this._dictionaryCacheVersion);
